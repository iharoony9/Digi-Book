...
export default function Customers() {
  ...
  const [formData, setFormData] = useState({ name: "", type: "customer", phone: "" });
  ...
  const handleSubmit = async () => {
    if (editingContact) {
      await updateContact.mutateAsync({ id: editingContact.id, ...formData, type: "customer" });
    } else {
      await createContact.mutateAsync({ ...formData, type: "customer", balance: 0 });
    }
    setIsAddOpen(false);
    ...
    <AppLayout>
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="sm:max-w-md p-6 rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-xl">{editingContact ? "Edit Customer" : "Add New Customer"}</DialogTitle>
          </DialogHeader>
